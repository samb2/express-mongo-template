...
export default class Repository<T extends Document> implements IRepository<T> {
    ...
    async find(where: object, options: IOption = {}, cache: ICache | undefined = undefined): Promise<any[]> {
        if (cache) {
            return this.model.find(where, {}, options).cacheQuery(cache);
        }
    ...
    async findOne(where: object, options: IOption = {}, cache: ICache | undefined = undefined): Promise<any> {
        if (cache) {
            return this.model.findOne(where, {}, options).cacheQuery(cache);
        }
    ...
    async findById(id: Types.ObjectId, options: IOption = {}, cache: ICache | undefined = undefined): Promise<any> {
        if (cache) {
            return this.model.findById(id, {}, options).cacheQuery(cache);
        }
    ...
    async paginate(
        ...
        if (cache) {
            results = await this.model.find(where, {}, { ...options, skip }).cacheQuery(cache);
        } else {
